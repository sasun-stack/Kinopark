...
const ARCHETYPE_LINES: Record<string, string> = {
  cinephile:
    "You don't watch films, you study them. Your last six tickets read like a syllabus.",
  "anime-devotee":
    "Half your favourite scenes are silent. You came for the worlds, not the words.",
  "drama-queen":
    "You buy popcorn anyway, even though you'll be too busy crying to eat it.",
  "horror-junkie":
    "You came alone, on purpose. The lights went down and you smiled.",
  "scifi-nerd":
    "Other people pick a film. You pick a future. Your watch history reads like research.",
  "action-hero":
    "Sound system on, lights down, phone face-down. You are the reason the back row exists.",
  sleuth: "You hate trailers because they spoil. You love long shots because they don't.",
  "hopeless-romantic":
    "You bought two tickets even though you came alone — old habit. Train scenes still hit.",
...
const BADGE_LINES: Record<string, string> = {
  "premium-baby":
    "And you book the recliner every single time. Of course you do.",
  marathoner: "Three films a week, easily. The staff know your order.",
  "date-night":
    "Always two tickets. Weekend, evening. The popcorn arrives without being asked.",
  ...
  "weekday-wizard":
    "Tuesday matinee, empty hall. You figured the secret out years ago.",
  "popcorn-loyalist":
    "Large salted, every time. The combo barely needs ordering.",
...
export async function GET(req: Request) {
  ...
  let badge: Badge;
  if (badgeOverride) {
    badge = BADGES.find((b) => b.id === badgeOverride) ?? BADGES[0];
  } else if (hasRealData) {
    const insights = analyzeTransactions(items);
    badge = pickBadgeFromInsights(insights);
  } else {
    badge = pickRandomBadge(phone);
  }
  ...
  if (hasRealData) {
    const insights = analyzeTransactions(items);
    const mockStats = buildStats(phone, archetype);
    stats = {
